...
unsigned char shuzi_zimo[15] =	//数字和字符字模
{
	...
	0xf5, 0x05, 0xb6, 0x97, 0x47, 0xd3, 0xf3, 0x85, 0xf7, 0xd7, 0x02, 0x70,
	    0x33, 0x67, 0x50
};
...
* Return         : 0-ok 1-fail
*******************************************************************************/
static unsigned char WritenDataVk2c21(unsigned char Addr,
				      unsigned char *Databuf, unsigned char Cnt)
{
	unsigned char n;
...
static long Vk2c21_ioctl(struct file *file, unsigned int cmd,
			 unsigned long args)
{
	...
	case IOCTL_CHAR_DISPLAY:
		if (args) {
			ret =
			    copy_from_user(display_arg, argp,
					   sizeof(display_arg) /
					   sizeof(display_arg[0]));
		}
		for (i = 0; i < 6; i++) {
			if (display_arg[i] <= '9' && display_arg[i] >= '0') {
				...
				vk2c21_dispram[i] |=
				    shuzi_zimo[pos_temp] & 0x0f;
				vk2c21_dispram[i] &= 0x8f;
				vk2c21_dispram[i] |=
				    shuzi_zimo[pos_temp] & 0xf0;
				Write1DataVk2c21(vk2c21_segtab[2 * i + 2] / 2,
						 vk2c21_dispram[i]);
			} else if (display_arg[i] <= 'z'
				   && display_arg[i] >= 'a') {
			} else if (display_arg[i] <= 'Z'
				   && display_arg[i] >= 'A') {
			}
